'''class JumpTrait:
    def __init__(self, entity):
        print("[INIT] JumpTrait instance created. Method reference:", self.jump)
    ...
    def __setattr__(self, name, value):
        if name == "jump" and isinstance(value, bool):
            print(f"[ALERT] JumpTrait.jump overwritten with {value}!")
            import traceback
...
class JumpTrait:
    def __init__(self, entity):
        print("[INIT] JumpTrait instance created. Method reference:", self.handle_jump)
        self.verticalSpeed = -12
        self.jumpHeight = 120
        self.entity = entity
        self.initalHeight = 384
    ...
    def handle_jump(self, jumping):
        ...
        if self.entity.inJump:
            if (
                (self.initalHeight - self.entity.rect.y) >= self.deaccelerationHeight
                or self.entity.vel.y == 0
            ):
                self.entity.inJump = False
                self.entity.obeyGravity = True
